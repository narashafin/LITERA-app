...
input[type=text],textarea{padding:11px 14px;border:2px solid #C7D8F8;border-radius:10px;font-family:'Nunito',sans-serif;font-size:.88rem;color:var(--navy);background:#fff;outline:none;transition:border-color .2s,box-shadow .2s;width:100%}
...
.badge-buku{display:inline-block;padding:3px 11px;border-radius:20px;font-size:.72rem;font-weight:700;background:#DBEAFE;color:#1E40AF}
...
<main class="main">
    <div class="page-header">
        ...
        <p><?= is_admin() ? 'Tambah, edit, dan hapus kategori buku perpustakaan.' : 'Daftar kategori buku yang tersedia di perpustakaan.' ?></p>
    </div>
    <div class="content">
        ...
        <div class="table-card">
            <div class="table-head">
                ...
                <form method="GET" class="search-form">
                    <input type="text" name="q" placeholder="Cari kode / nama…" value="<?= htmlspecialchars($search) ?>">
            ...
            <table>
                ...
                <tbody>
                    <?php if ($total === 0): ?>
                        <tr><td colspan="6" class="empty">Tidak ada kategori ditemukan.</td></tr>
                    <?php else:
                        $no = 1;
                        while ($row = mysqli_fetch_assoc($data)): ?>
                        <tr>
                            <td><?= $no++ ?></td>
                            <td><code style="background:#EFF6FF;color:var(--blue-dark);padding:2px 8px;border-radius:6px;font-size:.78rem;font-weight:700"><?= htmlspecialchars($row['kode']) ?></code></td>
